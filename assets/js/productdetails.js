...
function mostrarGaleria(product, galleryContainer) {
    ...
    warningImg.src = "./assets/images/control-warning/CartelAdvertencia.png";
...
function mostrarProductosSimilares(product, similarProducts, productCategory, productId) {
    ...
    if (container) {
        ...
        if (similares.length > 6) {
            viewAllLink.href = `./assets/pages/allproducts.html?category=${productCategory}&title=Controles Remotos de ${categoryMap[productCategory]}&idproduct=${productId}`;
        } else {
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            const allImages = [
                ...
                .filter(Boolean)
                .concat(
                    "./assets/images/control-warning/CartelAdvertencia.png?raw=true"
                );
